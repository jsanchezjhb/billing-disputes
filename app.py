...
# ── Cancellation screenshot images ───────────────────────────────────────────
import base64 as _b64
import os as _os

def _load_img(filename):
    """Load image as base64 from same directory as app.py."""
    try:
        app_dir = _os.path.dirname(_os.path.abspath(__file__))
        img_path = _os.path.join(app_dir, filename)
        with open(img_path, "rb") as f:
            return _b64.b64encode(f.read()).decode()
    except Exception:
        return None

_STEP1_IMG_B64 = _load_img("termination_step_1.png")
_STEP2_IMG_B64 = _load_img("termination_step_2.png")
...
def pdf_policy(dispute, loc):
    ...
    for label, b64_data in [
        ...
        img       = RLImage(img_buf, width=6.5*inch, height=5.0*inch)
        s.append(img)
        s.append(Spacer(1, 14))
